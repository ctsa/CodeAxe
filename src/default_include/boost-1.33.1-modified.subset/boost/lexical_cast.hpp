...
namespace boost
{
	...
	// \ctsa changing arg names to avoid shadow
        bad_lexical_cast(
            const std::type_info &ctsa_source_type,
            const std::type_info &ctsa_target_type) :
            source(&ctsa_source_type), target(&ctsa_target_type)
        {
        }
