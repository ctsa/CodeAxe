...
bool pool<UserAllocator>::release_memory()
{
  ...
  while (ptr.valid())
  {
    ...
    if (!all_chunks_free)
    {
      if (is_from(ctsa_free, ptr.begin(), ptr.element_size()))
      {
        std::less<void *> lt;
        ...
        do
        {
          prev_free = ctsa_free;
          ctsa_free = nextof(ctsa_free);
        } while (ctsa_free && lt(ctsa_free, end));
      }
    ...
    else
    {
      ...
      if (prev_free != 0)
        nextof(prev_free) = ctsa_free;
      else
        this->first = ctsa_free;
