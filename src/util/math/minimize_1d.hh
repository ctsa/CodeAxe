...
// $Id: minimize_1d.hh 931 2007-10-22 19:07:34Z ctsa $
...
get_computable_projection(FloatType& d_project,
  ...
  for(unsigned i(0);i<MAX_REDUCTION;++i){
    ...
    log_os << "WARNING: adjusting " << project_label << " from/to: "
...
#ifdef VERBOSE_DEBUG
  log_os << "bracket_minimum, init: f2=val(d2), d2: " << f2 << " " << d2 << "\n";
#endif
#else
  f2 = mf.val(d2);
#endif
...
#ifdef VERBOSE_DEBUG
    log_os << "bracket_minimum, f3=val(offset+wd3*scale), offset,wd3,scale,wd2,wd1: "
...
#ifdef VERBOSE_DEBUG
      log_os << "bracket_minimum, f4=val(offset+wd4*scale), offset,wd4,scale: "
             << f4 << " " << offset << " " << wd4 << " " << scale  << " "
             << r << " " << q << " " << wd4_max << " " << wd4_tmp << " "
             << wd3 << "\n";
#endif
#else
      const FloatType f4(mf.val(offset+wd4*scale));
#endif
...
brent_line_minimizer(FloatType d1,
...
#ifdef VERBOSE_DEBUG
    log_os << "brent_line_minimizer, f_next=val(d_next), d_next,d_low0,d_delta: "
...
#endif
#else
    const FloatType f_next(mf.val(d_next));
#endif
  ...
  throw math_util_exception("brent line minimizer failed to converge");
}
